...
function skillDraftCard(proposal) {
  if (!["draft_ready", "validated", "installed"].includes(proposal.status)) return `<div class="growth-proposal"><div><strong>${esc(proposal.skill_name)}</strong><small>${esc(titleCase(proposal.change_kind))} · ${esc(titleCase(proposal.status))}</small></div>${status(proposal.status)}</div>`;
